...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
	 ...
	 * the Adapter View is scrolled to the top and the mode is set to {@link Mode#PULL_FROM_START}. The default value is <var>true</var> if
	...
	/**
	 * Pass-through method for {@link PullToRefreshBase#getRefreshableView() getRefreshableView()}. {@link AdapterView#setAdapter(android.widget.Adapter)}
	 ...
	 * <p/>
	 * Please note, you do <strong>not</strong> usually need to call this method yourself. Calling setEmptyView on the AdapterView will automatically call this
	 * method and set everything up. This includes when the Android Framework automatically sets the Empty View based on it's ID.
	 * 
	 * @param newEmptyView
	 ...
	 * {@link AdapterView#setOnItemClickListener(OnItemClickListener) setOnItemClickListener(listener)}. This is just for convenience!
	 * 
	 * @param listener
	 ...
	 * the Adapter View is scrolled to the top and the mode is set to {@link Mode#PULL_FROM_START}
	 * 
	 * @param showIndicator
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			mIndicatorIvTop = new IndicatorLayout(getContext().getApplicationContext(), Mode.PULL_FROM_START);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
			params.rightMargin = getContext().getApplicationContext().getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
		...
		if (mode.showFooterLoadingLayout() && null == mIndicatorIvBottom) {
			// If the mode can pull down, and we don't have one set already
			mIndicatorIvBottom = new IndicatorLayout(getContext().getApplicationContext(), Mode.PULL_FROM_END);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
			params.rightMargin =getContext().getApplicationContext().getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			/**
			 * This check should really just be: mRefreshableView.getFirstVisiblePosition() == 0, but PtRListView internally use a HeaderView which messes the
			 * positions up. For now we'll just add one to account for it and rely on the inner condition which checks getTop().
			 */
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			/**
			 * This check should really just be: lastVisiblePosition == lastItemPosition, but PtRListView internally uses a FooterView which messes the
			 * positions up. For me we'll just subtract one to account for it and rely on the inner condition which checks getBottom().
			 */
