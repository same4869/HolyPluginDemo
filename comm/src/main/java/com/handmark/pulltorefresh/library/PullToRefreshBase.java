...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final String LOG_TAG = PullToRefreshBase.class.getSimpleName();
	...
	/**
	 * This is implemented by derived classes to return the created View. If you need to use a custom View (such as a custom ListView), override this method and
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			/**
			 * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer Views as they change often, which
			 * would negate any HW layer performance boost.
			 */
	...
	public static enum Mode {
		...
		/**
		 * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is either the Top or Left, depending on the scrolling
		 * direction.
		 */
		PULL_FROM_START(0x1),
		...
		/**
		 * Disables Pull-to-Refresh gesture handling, but allows manually setting the Refresh state via {@link PullToRefreshBase#setRefreshing()
		 * setRefreshing()}.
		 */
	...
	/**
	 * Listener that allows you to be notified when the user has started or finished a touch event. Useful when you want to append extra UI events (such as
	 * sounds). See ( {@link PullToRefreshAdapterViewBase#setOnPullEventListener}.
	 * 
	 * @author Chris Banes
	 */
	public static interface OnPullEventListener<V extends View> {
		 ...
		 * @param direction
		 *            - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the user is pulling. Only useful when
		 *            <var>state</var> is {@link State#PULL_TO_REFRESH} or {@link State#RELEASE_TO_REFRESH}.
		 */
	...
	/**
	 * An advanced version of the Listener to listen for callbacks to Refresh. This listener is different as it allows you to differentiate between Pull Ups,
	 * and Pull Downs.
	 * 
	 * @author Chris Banes
	 */
