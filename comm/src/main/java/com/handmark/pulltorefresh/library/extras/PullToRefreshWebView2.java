...
/**
 * An advanced version of {@link PullToRefreshWebView} which delegates the triggering of the PullToRefresh gesture to the Javascript running within the WebView.
 * This means that you should only use this class if:
 * <p/>
 * <ul>
 * <li>{@link PullToRefreshWebView} doesn't work correctly because you're using <code>overflow:scroll</code> or something else which means
 ...
 * <p/>
 * <p/>
 * The way this call works is that when a PullToRefresh gesture is in action, the following Javascript methods will be called: <code>isReadyForPullDown()</code>
 * and <code>isReadyForPullUp()</code>, it is your job to calculate whether the view is in a state where a PullToRefresh can happen, and return the result via
 * the callback mechanism. An example can be seen below:
 * <p/>
 * 
 * <pre>
