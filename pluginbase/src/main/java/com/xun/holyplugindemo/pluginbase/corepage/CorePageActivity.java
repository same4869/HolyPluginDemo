package com.xun.holyplugindemo.pluginbase.corepage;
...
import com.xun.holyplugindemo.pluginbase.corepage.core.CoreSwitchBean;
import com.xun.holyplugindemo.pluginbase.corepage.core.CoreSwitcher;
...
import static com.wenba.comm.APPUtil.getString;

public class CorePageActivity extends FragmentActivity implements CoreSwitcher, CorePageManager.OpenPageHandler,FragmentManager.OnBackStackChangedListener {
    ...
    // forresult 的fragment
    private com.xun.holyplugindemo.pluginbase.corepage.CorePageFragment mFragmentForResult = null;
        ...
        if (proxyLoader == null) {
            proxyLoader = new ClassLoader(CorePageActivity.class.getClassLoader()) {
                ...
                protected Class<?> findClass(String className) throws ClassNotFoundException {
                    BBLog.e("ljj", "CorePageActivity findClass == " + className);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            BBLog.e(TAG, "onCreate savedInstanceState");

            FragmentManager manager = this.getSupportFragmentManager();
            if (manager != null) {
                int count = manager.getBackStackEntryCount();
                BBLog.e(TAG, "onCreate fragment count : " + count);
                if (count > 0) {
    ...
    private void popOrFinishActivity() {
        ...
        for (int i = 0; i < getSupportFragmentManager().getBackStackEntryCount(); i++) {
            BBLog.i(TAG, this.toString() + " fragment in activity ==> " + getSupportFragmentManager()
    ...
    private void popBackStack() {
        com.xun.holyplugindemo.pluginbase.corepage.CorePageFragment fragment = getActiveFragment();
    ...
    public boolean findPage(String pageName) {
        int size = mActivities.size();
        BBLog.e(TAG, "findPage mActivities.size = " + size);
        int j = size - 1;
    ...
    public void removeUnlessFragment(List<String> fragmentLists) {
        ...
        if (manager != null) {
            ...
            for (int i = 0; i < count; i++) {// TO DEB FOR TEMP
                BBLog.i(TAG, "remove before fragment in activity ==> " + manager.getBackStackEntryAt(i).getName());
            }
            ...
            for (int i = 0; i < count1; i++) {// TO DEB FOR TEMP
                BBLog.i(TAG, "remove after fragment in activity ==> " + manager.getBackStackEntryAt(i).getName());
            }
            if (count == 0) {
