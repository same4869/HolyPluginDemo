...
public class NativeLibraryHelperCompat {
    ...
    private static int copyNativeBinariesBeforeL(File apkFile, File sharedLibraryDir) {
        try {
            ...
            return (int) com.xun.holyplugindemo.pluginbase.reflect.MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "copyNativeBinariesIfNeededLI", args);
    ...
    private static int copyNativeBinariesAfterL(File apkFile, File sharedLibraryDir) {
        try {
            Object handleInstance = com.xun.holyplugindemo.pluginbase.reflect.MethodUtils.invokeStaticMethod(handleClass(), "create", apkFile);
            ...
            if (isVM64(apkFile.getAbsolutePath())) {
                if (Build.SUPPORTED_64_BIT_ABIS.length > 0) {
                    ...
                    int abiIndex = (int) com.xun.holyplugindemo.pluginbase.reflect.MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "findSupportedAbi", handleInstance, Build.SUPPORTED_64_BIT_ABIS);
                    if (abiIndex >= 0) {
                ...
                if (Build.SUPPORTED_32_BIT_ABIS.length > 0) {
                    ...
                    int abiIndex = (int) com.xun.holyplugindemo.pluginbase.reflect.MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "findSupportedAbi", handleInstance, Build.SUPPORTED_32_BIT_ABIS);
                    if (abiIndex >= 0) {
            ...
            args[1] = sharedLibraryDir;
            args[2] = abi;
            return (int) com.xun.holyplugindemo.pluginbase.reflect.MethodUtils.invokeStaticMethod(nativeLibraryHelperClass(), "copyNativeBinaries", args);
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
