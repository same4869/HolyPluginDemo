...
import com.xun.holyplugindemo.pluginbase.core.NativeLibraryHelperCompat;
...
public class PluginInstallHandler {
    ...
    private static int installPlugin(Context appContext, InputStream stream, String pluginName) {
        BBLog.d(TAG, pluginName + "install plugin start .........");
        ...
        try {
            ...
            BBLog.d(TAG, pluginName + "install plugin success .........");
        } catch (Exception e) {
    ...
    static boolean installUpdate(Context appContext, File file) {
        ...
        if (!isPluginPermitted(appContext, file)) {// TODO 插件文件不合法
            BBLog.e(TAG, file.getAbsolutePath() + " 不合法，安装失败");
            FileUtils.deleteQuietly(file);
            return ret;
        } else {
            BBLog.e(TAG, file.getAbsolutePath() + "合法，校验成功");
        }

        if (PluginManager.isPluginLoad(updateApkInfo.getPluginName())) {
            BBLog.d(TAG,updateApkInfo.getPluginName()+"已载入内存，新版本被禁止安装");
            return ret;
        }
    ...
    private static boolean publicKeyIsValid(Context appContext, PackageInfo mainPackageInfo, File apkFile) throws
        ...
        BBLog.e(TAG, "hostKey=" + hostKey);
        BBLog.e(TAG, "pluginKey=" + pluginKey);
